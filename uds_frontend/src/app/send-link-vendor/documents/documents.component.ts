 ...
 updateDocument(){
  ...
  this.vendorService.updateDocument(this.vendor_id,formData).subscribe((res:any)=>{
    ...
    if(res){
      ...
      this.route.navigate(['/master/vendor/vendor-management/vendor-list'])
    }
  })
}
...
addDocs(){
  ...
  var formData: any = new FormData();
  console.log('image',this.imageToUpload);
    ...
    if (result.value) {
      ...
      this.vendorService.updateStatus(this.vendor_id, data).subscribe((res:any)=>{
        ...
        if(res.code ==200){
          ...
          this.route.navigate(['/master/vendor/vendor-management/vendor-list/unapproved-list'])
        }
      })
