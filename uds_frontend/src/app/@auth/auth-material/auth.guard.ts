      ...
      return new Observable<boolean | UrlTree>((observer) => {
        this._authService.getAllAccess_Modules(singInUser?.employee_id).subscribe(
          (res: any) => {
            ...
            const includesDashboard = state?.url.includes('dashboard');
            const includesSendRfpLink = state?.url.includes('send-rfp-link');
            if (!(includesDashboard || includesSendRfpLink)) {
              // For module access
