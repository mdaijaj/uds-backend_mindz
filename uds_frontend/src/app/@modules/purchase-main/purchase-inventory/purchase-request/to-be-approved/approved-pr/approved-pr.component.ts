...
export class ApprovedPrUpdateComponent {
  ...
  getByPr() {
    ...
    this.prService.getByIdPR(data).subscribe((res: any) => {
      ...
      let employee_id = this.singlePrData?.employee_id;
      if (employee_id) {
  ...
  budget_details() {
    ...
    let data = {
      dept_name: this.singlePrData.department,
      type: this.singlePrData.PR_type,
      financial_year: this.singlePrData.financial_year,
    }
  ...
  addRemark(e: any) {
    ...
    if (this.statusPr == "BOM PR") {
      let finalremainingAmount = this.remainingAmount + this.singlePrData.total_mvp

      if (finalremainingAmount < this.totalAmountPrice) {
        this.openBudgetRemainder();
        return;
      }
      ...
      if (finalremainingAmount < this.totalAmountService) {
        this.openBudgetRemainder();
        return;
      }
      ...
      if (finalremainingAmount < this.totalAmountItem) {
        this.openBudgetRemainder();
        return;
      }
    ...
    const dataEdit = {
      ...
      final_quantity: "800",
      final_MVP: this.totalAmountPrice || this.totalAmountItem || this.totalAmountService,
      status: this.statusCategory,
      totelMVP: this.totalAmountPrice || this.totalAmountItem || this.totalAmountService
    }
