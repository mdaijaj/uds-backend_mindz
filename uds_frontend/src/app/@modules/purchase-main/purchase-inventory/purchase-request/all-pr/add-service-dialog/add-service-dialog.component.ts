...
export class AddServiceDialogComponent {
  ...
  selectCateGory(e: any) {
    ...
    let data = {
      category_id: e.value,
      workflow_type: this.cellData.type,
      department_name: this.cellData.department_name,
    }
    this.prService.getCategoryByWorkFlowList(data).subscribe((res: any) => {
      if (res.data) {
        this.prService.getAllServiceMaster().subscribe((res: any) => {
          this.all_prod_data = res.data.filter((res: any) => res.service_category_id == this.cat_id)
          // this.filteredData = this.all_prod_data;
          this.all_prod_data.forEach((pro: any) => {
            pro['quantity'] = this.quantity
          });
