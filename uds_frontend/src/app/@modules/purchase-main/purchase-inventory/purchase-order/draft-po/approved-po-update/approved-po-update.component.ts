...
@Component({
  selector: 'app-approved-po-update',
  templateUrl: './approved-po-update.component.html',
  styleUrls: ['./approved-po-update.component.scss']
})
export class ApprovedPoUpdateComponent {
  ...
  ngOnInit() {
    ...
    this.activeroute.queryParams.subscribe((params: any) => {
      ...
      this.id = params?.po_id;
      this.po_type = params?.po_type;
      // this.getByPr();
    });
    ...
    if (this.id) {
      ...
      this.prService.getByIdPO(this.id, data).subscribe((res: any) => {
        this.singlePoData = res.data[0];
        console.log(this.singlePoData, ' this.singlePoData');

        if (this.po_type == "Item PO") {
          this.CF_1['po_attached_doc'].setErrors(null);
          while (this.CF_1.item_detail.length > 0) {
            this.CF_1.item_detail.removeAt(0);
          }
          this.createPrForm.patchValue({
            branchName: this.singlePoData?.tbl_branch.branch_name,
            branchCode: this.singlePoData?.tbl_branch.branch_code,
            gstNo: this.singlePoData?.tbl_branch.branch_gstnumber,
            branchAddress: this.singlePoData?.tbl_branch.branch_address,
            delivery_branch: this.singlePoData?.delivery_branch,
            department_name: this.singlePoData?.department_name,
            po_type: this.singlePoData?.po_type,
            financial_year: this.singlePoData?.financial_year,
            po_date: this.singlePoData?.po_date,
            po_number: this.singlePoData?.po_number,
            po_supplier_vendor_name: this.singlePoData?.po_supplier_vendor_name,
            po_remark: this.singlePoData?.po_remark,
            item_detail: this.singlePoData?.po_item_details
          });
          const itemArray = this.createPrForm.get('item_detail') as FormArray;
          this.singlePoData?.po_item_details.forEach((itemSpec: any) => {
            const newRow = this.fb.group({
              item_id: itemSpec.item_id,
              item_document: itemSpec.ItemMaster.item_document,
              item_name: itemSpec.ItemMaster.item_name,
              item_code: itemSpec.ItemMaster.item_code,
              item_quantity: itemSpec.item_quantity,
              unit_price: itemSpec.unit_price,
              amount: itemSpec.amount,
              tax: itemSpec.tax,
              asset_category_id: itemSpec.asset_category_id,
            });
            itemArray.push(newRow);
          });
          this.budget_details();
          this.totalAmountItem = this.singlePoData?.total
        }
        if (this.po_type == "Service PO") {
          this.CF_2['po_attached_doc'].setErrors(null);
          while (this.CF_2.service_detail.length > 0) {
            this.CF_2.service_detail.removeAt(0);
          }
          this.createServiceForm.patchValue({
            ...
            po_supplier_vendor_name: this.singlePoData?.po_supplier_vendor_name,
            po_remark: this.singlePoData?.po_remark,
            service_detail: this.singlePoData?.po_service_details
          });
          const itemArray = this.createServiceForm.get('service_detail') as FormArray;
          this.singlePoData?.po_service_details.forEach((serviceSpec: any) => {
            const newRow = this.fb.group({
              service_id: serviceSpec.po_service_id,
              service_document: serviceSpec.tbl_servicemaster.service_document,
              service_name: serviceSpec.tbl_servicemaster.service_name,
              service_code: serviceSpec.tbl_servicemaster.service_code,
              service_quantity: serviceSpec.service_quantity,
              unit_price: serviceSpec.unit_price,
              amount: serviceSpec.amount,
              tax: serviceSpec.tax,
              service_category_id: serviceSpec.service_category_id,
            });
            itemArray.push(newRow);
          });
          this.budget_details();
          this.totalAmountService = this.singlePoData?.total
        }
  ...
  budget_details() {
    let val = this.po_type == 'Item PO' ? this.createPrForm.value : this.createServiceForm.value
  ...
  addProduct(e: any) {
    if (this.createPrForm.value.department_name) {
      let data = {
        type: "Item PO",
        department_name: this.createPrForm.value.department_name
      }
      e.preventDefault();
      ...
      this.toast.warning("Pls Fill Department!");
    }
  }

  addService(e: any) {
    if (this.createServiceForm.value.department_name) {
      let data = {
        type: "Service PO",
        department_name: this.createServiceForm.value.department_name
      }
      e.preventDefault();
  ...
  keyUpService(event: any, i: any, data: any) {
    ...
    let list = control.value;
    for (let a = 0; a <= i; a++) {
      list[i].amount = list[i]?.service_quantity * data?.value?.unit_price
    }
    // list.map((item: any) => {
    //   // itemData
    //   console.log(item)
  ...
  onSubmit() {
    ...
    if (this.po_type == 'Item PO') {
      ...
      if (this.budgetId <= 0) {
        this.toast.warning("Pls Fill For Suitable Budget!");
        return
      }
    ...
    if (this.po_type == 'Item PO') {
      ...
      formData.append('budget_id', this.budgetId);
      formData.append('branch_id', this.singlePoData.branch_id);
      ...
      this.prService.updatePo(this.id, formData).subscribe((res: any) => {
    ...
    if (this.po_type == 'Service PO') {
      ...
      formData2.append('budget_id', this.budgetId);
      formData2.append('branch_id', this.singlePoData.branch_id);
      ...
      this.prService.updatePo(this.id, formData2).subscribe((res: any) => {
        this.resdata = res.data;
        if (res.code == 200) {
          this.toast.success(res.message);
          this.route.navigate([
  ...
  addRemark(e: any) {
    ...
    const dialogRef = this.dialog.open(RemarkDilogComponent, {
      width: '400px',
      data: { cellData: this.singlePoData },
    });
    dialogRef.afterClosed().subscribe((result) => {
