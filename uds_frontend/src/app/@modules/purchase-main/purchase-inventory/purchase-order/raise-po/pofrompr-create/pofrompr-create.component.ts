...
@Component({
  selector: 'app-pofrompr-create',
  templateUrl: './pofrompr-create.component.html',
  styleUrls: ['./pofrompr-create.component.scss']
})
export class PofromprCreateComponent {
  ...
  constructor(
    ...
    this.createPrForm = this.fb.group({
      ...
      item_detail: new FormArray([new FormGroup({
    ...
    this.createServiceForm = this.fb.group({
      ...
      po_supplier_vendor_name: new FormControl(null, [Validators.required]),
      employee_id: new FormControl(null,),
      service_detail: new FormArray([new FormGroup({
        ...
        service_quantity: new FormControl(null, [Validators.required]),
        unit_price: new FormControl(0, [Validators.required]),
        amount: new FormControl(null, [Validators.required]),
        tax: new FormControl(null, [Validators.required]),
  ...
  ngOnInit() {
    ...
    this.activeroute.queryParams.subscribe((params: any) => {
      ...
      if (params?.pr_type == "Service PR") {
        this.po_type = "Service PO";
      } else if (params?.pr_type == "Item PR") {
        this.po_type = "Item PO";
      }
      this.pr_type = params?.pr_type;
      // this.getByPr();
    });

    if (this.id) {
      let data = {
        PR_categories: this.pr_type,
      ...
      this.prService.getByIdPR(data).subscribe((res: any) => {
        ...
        if (this.po_type == "Item PO") {
          ...
          this.createPrForm.patchValue({
            department_name: this.singlePoData?.department,
            po_type: this.singlePoData?.PR_type,
            financial_year: this.singlePoData?.financial_year,
            item_detail: this.singlePoData?.procurement_item_requests
          });
          const itemArray = this.createPrForm.get('item_detail') as FormArray;
          this.singlePoData?.procurement_item_requests.forEach((itemSpec: any) => {
            const newRow = this.fb.group({
              item_id: itemSpec.item_id,
              item_document: `${environment.servralUrl + '/' + itemSpec.ItemMaster.item_document}`,
        ...
        if (this.po_type == "Service PO") {
          ...
          this.createServiceForm.patchValue({
            ...
            po_type: this.singlePoData?.PR_type,
            financial_year: this.singlePoData?.financial_year,
            service_detail: this.singlePoData?.procurement_service_requests
          });
          const itemArray = this.createServiceForm.get('service_detail') as FormArray;
          this.singlePoData?.procurement_service_requests.forEach((serviceSpec: any) => {
            const newRow = this.fb.group({
              service_id: serviceSpec.service_id,
              service_document: `${environment.servralUrl + '/' + serviceSpec.tbl_servicemaster.service_document}`,
        ...
        this._configurationalMasterService.getListBranchSetup().subscribe((res: any) => {
          ...
          this.billingbranchList = res.data.filter((res: any) => res.billing_status == 1);
        })
  ...
  onSubmit() {
    ...
    // this.totalPRValue = this.calculateTotalValue(this.totalAmountItem);
    ...
    if (this.po_type == 'Item PO') {
      ...
      formData.append('branch_id', val.branch_id);
      ...
      formData.append('procurement_id', this.singlePoData.procurement_id);
      this.prService.createPoFromPr(formData).subscribe((res: any) => {
    ...
    if (this.po_type == 'Service PO') {
      ...
      formData2.append('branch_id', val2.branch_id);
      ...
      formData2.append('procurement_id', this.singlePoData.procurement_id);
      this.prService.createPoFromPr(formData2).subscribe((res: any) => {
        this.resdata = res.data;
        if (res.code == 200) {
          this.toast.success(res.message);
          this.route.navigate([
