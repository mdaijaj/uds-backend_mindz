...
@Component({
  selector: 'app-product-asset-update',
  templateUrl: './product-asset-update.component.html',
  styleUrls: ['./product-asset-update.component.scss'],
})
export class ProductAssetUpdateComponent {
  status = [
    { id: 1, name: 'Assigned' },
    { id: 2, name: 'Open' },
    { id: 3, name: 'Dispossed/Scraped' },
    { id: 4, name: 'Lost' },
    { id: 5, name: 'Issue' },
  ];
  ...
  constructor(
    ...
    this.productupdateForm = this.fb.group({
      ...
      purchased_date: new FormControl(null, [Validators.required]),
      vendor_name: new FormControl(null, [Validators.required]),
      purchased_cost: new FormControl(null, [Validators.required]),
  ...
  onSubmit() {
    ...
    this.assetService.crateProductAsset(value).subscribe((res: any) => {
      ...
      if (res.code == 200) this.toastr.success(res.messege);
      this.route.navigate([
        'master/assest-management/assest-management-sub/product-asset-master',
      ]);
    });
  }
