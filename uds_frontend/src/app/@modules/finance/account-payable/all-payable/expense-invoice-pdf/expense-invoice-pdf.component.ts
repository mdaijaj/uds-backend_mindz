...
@Component({
  selector: 'app-expense-invoice-pdf',
  templateUrl: './expense-invoice-pdf.component.html',
  styleUrls: ['./expense-invoice-pdf.component.scss']
})
export class ExpenseInvoicePdfComponent {
  leadId: any;
  singleLeadData: any;
  idZogo: string | null;
  invoiceData: any;
  ...
  ngOnInit(): void {
    this.activeroute.queryParams.subscribe((params: any) => {
      ...
      this.zohoId = Number(params.expenseZohoId)
  ...
  getByZoho() {
    this._finaceService.getByExpenseZohoPayble(this.zohoId).subscribe((res: any) => {
      console.log(res, 'ressss');
      this.invoiceData = res.data;
      console.log(this.invoiceData, 'this.invoiceData');
      this.brNumberInvoice = res.data.br_number;
      this.leadId = res.data.lead_genration_id;
  ...
  verificationTable() {
    this.leadService.get_invoice_list().subscribe((res: any) => {
      ...
      console.log(this.totalNetAmount, 'this.totalNetAmount');
      console.log(this.invoiceData?.finalAmount,'this.invoiceData?.finalAmount');
      this.finalAMountCopy=Number(this.invoiceData?.finalAmount)
      this.igstAmount = this.finalAMountCopy * 18 / 100;
      console.log(this.igstAmount, 'this.igstAmount');

      this.totlepayableAmount = this.finalAMountCopy + this.igstAmount;
