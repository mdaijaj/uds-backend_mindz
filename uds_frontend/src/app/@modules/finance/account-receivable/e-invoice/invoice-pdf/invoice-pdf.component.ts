...
export class InvoicePdfComponent {
  ...
  getPatchData(){
    this.leadService.getzohoByIdData(this.zohoId).subscribe((res:any)=>{
      this.singleLeadData = res.data,
      this.dataBrSame = this.singleLeadData.item_details;
      for (let item of this.dataBrSame) {
        this.totalNetAmount += Number(item.netAmount || item.net_amount)
