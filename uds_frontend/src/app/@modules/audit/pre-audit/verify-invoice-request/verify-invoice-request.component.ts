...
@Component({
  selector: 'app-verify-invoice-request',
  templateUrl: './verify-invoice-request.component.html',
  styleUrls: ['./verify-invoice-request.component.scss']
})
export class VerifyInvoiceRequestComponent {
  toDate: any;
  id: any;
  ...
  loginUser: any;
  now = new Date();
  formattedDate = this.now.toLocaleDateString();
  ...
  constructor(
    ...
    this.leadForm = this.fb.group({
      ...
      mobile_number: new FormControl(null, [Validators.maxLength(10),
  ...
  patchFormvalue() {
    ...
    this.leadForm.patchValue({
      ...
      categories: this.singleLeadData?.categories_name,
      regional_bussiness_lead: this.singleLeadData?.regional_bussiness_lead_name,
      ...
      // audit_start_date: this.leadForm.value.training_start_date,//
      audit_start_date: moment(this.singleLeadData.training_start_date).format('YYYY-MM-DD'),
      audit_end_date: moment(this.singleLeadData.training_end_date).format('YYYY-MM-DD'),
      // audit_end_date: this.leadForm.value.training_end_date,
      // approver_name_l1Id:this.singleLeadData?. lead_created_by_name,
      approver_name_l1Id: Number(this.employee_id),
      wo_verified_on:this.formattedDate,
      wo_verify_by:this.loginUserName,
      quotation_currency: Number(this.singleLeadData?.quotation_currency),
      ...
      quote_prepared_date:moment(this.singleLeadData?.quote_prepared_date).format('YYYY-MM-DD'),
      l1_verifer_name:this.singleLeadData?.opp_verifier_name_level1,
      l1_verifer_date:moment(this.singleLeadData?.opp_verified_date_level1).format('YYYY-MM-DD'),
      remarsk_l1:this.singleLeadData?.opp_verified_remarks_level1,
      l2_verifer_name:this.singleLeadData?.opp_verifier_name_level2,
      l2_verifer_date:moment(this.singleLeadData?.opp_verified_date_level2).format('YYYY-MM-DD'),
      remark_l2:this.singleLeadData?.opp_verified_remarks_level2,
  ...
  submitForm() {
    ...
    const datas = {
      wo_verifyOn: this.leadForm.value.wo_verified_on,//
      wo_verifyBy: this.leadForm.value.wo_verify_by,//
      wo_verification_status: this.leadForm.value.wo_verification_status,//
      CI_assessment_fee_comment1: this.leadForm.value.comment1,//
      digitallySignedSoftcopy_comment2: this.leadForm.value.comment2,//
      travelLivingClaim_comment3: this.leadForm.value.comment3,//
      TL_supportingNotSent_comment4: this.leadForm.value.comment4,//
      invoiceIsWithGST_comment5: this.leadForm.value.comment5,//
      ClientNamePO_comment6: this.leadForm.value.comment6,//
      audit_start_date: this.leadForm.value.audit_start_date,//
      audit_end_date: this.leadForm.value.audit_end_date,//
      ICTDate: this.leadForm.value.ICT_Date,//
      // br_lists: this.leadForm.value.br_lists,
      associatedCompany: this.leadForm.value.ociatedCompany,//
      br_number: Number(this.leadForm.value.BrNumber),//
      email: this.leadForm.value.email,//
      designation: this.leadForm.value.jobTitle,//
      firstName: this.leadForm.value.firstName,//
      city: this.leadForm.value.city,//
      state: this.leadForm.value.state,
      mobilePhonenumber: this.leadForm.value.Mob_number,//
      streetAddress: this.leadForm.value.streetAddress,//
      addressLine2: this.leadForm.value.addressLine,//
      GSTNumber: this.leadForm.value.gstNumber,//
      creditDays: this.leadForm.value.creditDay,//
      lead_genrate_id: Number(this.lead_id),//
      billing_site_copy:this.singleLeadData.billing_site_copy,
      
    }
    this.leadService.update_invoice_verification_comments(datas).subscribe((res: any) => {
      
      
      this.toast.success("Invoice sent to Finance Successfully..")
      this.route.navigate(['master/audit/pre-audit/invoice-verify-list'],
      {
        queryParams: { lead_genration_id: this.lead_id, br_number: this.br_numbers }
      }
    );
    },
    error => {
  ...
  billingAddress(e: any) {
    ...
    this.leadService.get_ById_BR_Number(e.value).subscribe((res: any) => {
      ...
      this.br_numbers = res.data[0]?.br_number
      ...
      this.br = res?.data[0]?.br_number,
        ...
        this.leadChiled=res.data[0]?.lead_genration_id
        this.leadForm.patchValue({
          associatedCompany: res.data[0]?.associated_company,
          BrNumber: res.data[0]?.br_number,
          Mob_number: res.data[0]?.mobile_number,
          streetAddress: res.data[0]?.street_address,
          gstNumber: res.data[0]?.gst_number,
          creditDay: res.data[0]?.remaining_count,
          city: res.data[0]?.city_name,
          state: res.data[0]?.state_name,
          email: res.data[0]?.email,
          jobTitle: res.data[0]?.job_title,
          firstName: res.data[0]?.first_name,
          addressLine: res.data[0]?.address2,
        })
    })

    this.leadService.getByIdLead(this.lead_id).subscribe((res: any) => {
    this.leadDataChiled = res.data;
   console.log(this.leadDataChiled,'this.leadDataChiled');
   this.mobiles = res.data.mobile_number;
   console.log(this.mobiles,'this.mobiles');
   this.mail=res.data.email;
   console.log(this.mail,'this.mail');
   this.designation=res.data.job_title;
   console.log(this.designation,'this.designation');
   this.address2=res.data.address2;

   this.leadForm.patchValue({
    associatedCompany: res.data.associated_company,
    gstNumber: res.data.gst_number,
    Mob_number: res.data.mobile_number,
    streetAddress: res.data.street_address,
    creditDay: res.data.remaining_count,
    addressLine:res.data.address2
