...
@Component({
  selector: 'app-upload-verify-csv-popup',
  templateUrl: './upload-verify-csv-popup.component.html',
  styleUrls: ['./upload-verify-csv-popup.component.scss']
})
export class UploadVerifyCsvPopupComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.DemoFileUrl=`${this.databaseKey}/api/v1/download_Document_lead_file/document_link-1690199837645.xlsx`
  ...
  upload_fileS() {
    ...
    if (this.selectedFile) {
      ...
      this.leadService.postAuditVeriusDataPatch(this.lead_id,formData).subscribe(res => {
        this.CSV_file = res;
