...
@Component({
  selector: 'app-payment-verification-check',
  templateUrl: './payment-verification-check.component.html',
  styleUrls: ['./payment-verification-check.component.scss']
})
export class PaymentVerificationCheckComponent {
  ...
  submitForm(auditor_status: string) {
    ...
    this.leadService.editLead(this.audit_postId, data).subscribe((res: any) => {
      ...
      this.route.navigate(
        ['master/audit/post-audit/rbh_verification'],
