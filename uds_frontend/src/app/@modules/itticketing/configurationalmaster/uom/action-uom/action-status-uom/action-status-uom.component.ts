...
@Component({
  selector: 'app-action-status-uom',
  templateUrl: './action-status-uom.component.html',
  styleUrls: ['./action-status-uom.component.scss']
})
export class ActionStatusUomComponent {
  assignAction: any;
  singleData: any = {};
  isChecked = false;
  checkBox: any;
  ...
  toggle(e: any, isChecked: boolean) {
    try {
      ...
      this._configurationalMasterService.statusUOMById(this.cellValue, body).subscribe((response: any) => {
        if (response) {
