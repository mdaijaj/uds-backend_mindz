...
@Component({
  selector: 'app-level-slab-status',
  templateUrl: './level-slab-status.component.html',
  styleUrls: ['./level-slab-status.component.scss']
})
export class LevelSlabStatusComponent {
  assignAction: any;
  singleData: any = {};
  isChecked = false;
  checkBox: any;
  ...
  toggle(e: any, isChecked: boolean) {
    try {
      ...
      this._configurationalMasterService.updateStatusLevelSlab(this.cellValue, body).subscribe((response: any) => {
        if (response) {
