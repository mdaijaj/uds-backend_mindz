...
@Component({
  selector: 'app-grade-status',
  templateUrl: './grade-status.component.html',
  styleUrls: ['./grade-status.component.scss']
})
export class GradeStatusComponent {
  assignAction: any;
  singleData: any = {};
  isChecked = false;
  checkBox: any;
  ...
  toggle(e: any, isChecked: boolean) {
    try {
      ...
      this._configurationalMasterService.updateGradeStatus(this.cellValue, body).subscribe((response: any) => {
        if (response) {
