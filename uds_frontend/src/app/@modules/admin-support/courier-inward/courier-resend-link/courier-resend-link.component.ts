...
@Component({
  selector: 'app-courier-resend-link',
  templateUrl: './courier-resend-link.component.html',
  styleUrls: ['./courier-resend-link.component.scss']
})
export class CourierResendLinkComponent {
  rowClass: any;
  ...
  updateInward() {
    let val = this.create_inward_Form.value;
      let data ={ 
        ...
        redirect_url: window.location.origin +'/master/admin-support/courier-inward/courier-accept-reject'
      }
      this.adminService
      .courier_Resend(this.emp_id, data)
      .subscribe((res: any) => {
        if (res.code == 200) {
          this.route.navigate([
