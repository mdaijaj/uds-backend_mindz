...
@Component({
  selector: 'app-create-hk-s-agreement',
  templateUrl: './create-hk-s-agreement.component.html',
  styleUrls: ['./create-hk-s-agreement.component.scss'],
})
export class CreateHkSAgreementComponent {
  rowClass: any;
  private gridApi!: GridApi<any>;
  create_amc_Form: FormGroup;
  inward_id: any;
  id: any;
  public dis = [{ id: 1, name: 'Office Boy or Security' }];
  public status = [
    { id: 1, name: 'Open' },
    { id: 2, name: 'Closed' },
    { id: 3, name: 'Rejected' },
  ...
  constructor(
    ...
    this.create_amc_Form = this.fb.group({
      name_of_agreement: new FormControl(null, [Validators.required]),
      ...
      description_of_agreement: new FormControl(null, [Validators.required]),
      count_of_employees: new FormControl(null, [Validators.required]),
      ...
      hk_status: new FormControl(null, [Validators.required]),
  ...
  ngOnInit(): void {
    ...
    this.adminService.get_ById_hk(this.id).subscribe((res: any) => {
      ...
      this.create_amc_Form.patchValue({
        ...
        company_name: this.single_amc.company_name,
        ...
        description_of_agreement: this.single_amc.description_of_agreement,
        count_of_employees: this.single_amc.count_of_employees,
        ...
        hk_status: this.single_amc.hk_status,
  ...
  createAmc() {
    if (this.create_amc_Form.value.hk_status == 'Rejected') {
      ...
      formData.append('count_of_employees', val.count_of_employees);
      ...
      formData.append('hk_status', val.hk_status);
      ...
      this.adminService.creat_hk(formData).subscribe((res: any) => {
        if (res.code == 200) {
          this.toast.success(res.message);
          this.res_id = res.data.security_agreement_id;
          console.log('data', this.res_id, this.id);
        }
      });
    }
  }
  ...
  updateAmc() {
    ...
    formData.append('terminate_copy_upload', this.imageTermToUpload);
    this.adminService.update_hk(this.id, formData).subscribe((res: any) => {
      if (res.code == 200) {
        this.toast.success(res.message);
      }
    });
  ...
  uploadOtherDoc(i: any, control: any) {
    ...
    if (!this.res_id && !this.id) {
      this.toast.error('Please create HKs first');
    }
    if (
      control.value.file_name === null &&
      control.value.upload_agreement_copy === null
    ) {
    ...
    if (file) {
      ...
      this.adminService
        .edit_hk_doc(this.res_id || this.id, formData)
        .subscribe((res: any) => {
  ...
  createSucces() {
    ...
    this.route.navigate([
      '/master/admin-support/house-keeping-sicurity-agreement/house-keeping-security-agreement-list',
    ]);
