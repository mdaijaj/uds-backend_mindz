...
export class FeaApprovelCreateComponent {
  ...
  dataMain: {
    ...
    opp_verified_remarks_level1: any; reject_remarks_a: any; reject_remarks_b: any; opp_verifier_name_level2: any; opp_verified_date_level2: string; opp_verified_remarks_level2: any; quotation_currency: number;totalAmount_inInr:any; slab_quote: any; logo_cost: any; quote_prepared: any; quote_prepared_date: string; status: string; company_logo_req: any; company_logo_cost: any;
    agreed_slab_a: any;
    agreed_slab_b: any;
    ...
    fea_approval_date: any,
    br_number: any,
    customer_name: any,
    approval_type: any,
    revenue_Value: any;
    adv_payment_date: any;
    ROH_RSM: any;
    fea_approval_status:any
    // csp_auditor_name: any;
    calculatedTotalAmmountWithStages: any;
    feaApprovalFlag: boolean;
    lead_genration_id: any;
    status3:any;
    multiple_site_audit:any
  };
  ...
  constructor(
    ...
    this.leadForm = this.fb.group({
      ...
      fea_approval_date: new FormControl(null, Validators.required),
      br_number: new FormControl(null, Validators.required),
      customer_name: new FormControl(null, Validators.required),
      approval_type: new FormControl(null, Validators.required),
      revenue_Value: new FormControl(null, Validators.required),
      outstanding_amount: new FormControl(0, Validators.required),
      adv_payment_date: new FormControl(null, Validators.required),
      // csp_auditor_name: new FormControl(null, Validators.required),
      ROH_RSM: new FormControl(null, Validators.required),
      fea_approval_status: new FormControl(null, Validators.required),
      advance_payment_remarks: new FormControl(null, Validators.required),
  ...
  ngOnInit() {
    ...
    this.activeroute.queryParams.subscribe((params: any) => {
      ...
      this.leadService.getByIdLead(this.lead_id).subscribe((res: any) => {
        ...
        this.contactOwner= this.loginUser.first_name
       ...
       if(res && res.data){
        ...
        if (this.singleLeadData?.certificate_type_name === "Non-Cert") {
          ...
          this.leadForm.patchValue({
            approval_type: this.singleLeadData.non_cert_type,
          })

        } else {
          this.nonCertTrue = true;
          this.leadForm.patchValue({
            approval_type: this.singleLeadData.product_request,
          })
        }
      }
        this.nonCertFunc();
  ...
  getAchievementList() {
    this.recruitService.getAllAchivement().subscribe(
      (res: any) => {
        ...
        this.variables = this.allAchievement.filter((fil:any)=>fil.role_master_id==4 || fil.role_master_id==22 || fil.role_master_id==27).map((res: any) => res.first_name)
        this.checkData = this.allAchievement.filter((fil:any)=>fil.role_master_id==4 || fil.role_master_id==22 || fil.role_master_id==27)
  ...
  patchFormvalue() {
    ...
    this.leadForm.patchValue({
      ...
      fea_approval_date: this.myDate,
      customer_name: this.singleLeadData?.associated_company,
      phone_code:this.singleLeadData?.userrespectiveleads[0]?.phone_code,
      ROH_RSM: this.singleLeadData?.ROH_RSM,
      advance_payment_remarks:this.singleLeadData?.advance_payment_remarks,
      adv_payment_date:this.singleLeadData?.adv_payment_date
  ...
  submitForm() {
    ...
    // }
    let dynamicStatus
    let feaDynamicStatus
    if(this.singleLeadData.status == 'Documents Signed'){
        dynamicStatus = 'Documents Signed';
    }else{
      dynamicStatus ="Digitally Signed Document";
    }

    if(this.role == 'order_booking' || this.leadForm.value.fea_approval_status == 'Reject'){
      feaDynamicStatus = 'Sent FEA Approval Pending'
    }else{
      feaDynamicStatus = 'Sent FEA Approval'
    }

    if (this.leadForm.value.approval_type == null) {
      this.toast.error('Approval Type should not be empty.');
      return;
    }
    if (this.leadForm.value.fea_approval_status == null && this.role != 'order_booking') {
      this.toast.error('Please select approval status.');
      return;
    }
    if (this.leadForm.value.outstanding_amount == null) {
      this.toast.error('Outstanding Amount should not be empty.');
      return;
    }
    // if (this.leadForm.value.csp_auditor_name == null) {
    //   this.toast.error('CSP/SP/Auditor Name should not be empty.');
    //   return;
    // }
    if (this.leadForm.value.adv_payment_date == null) {
      this.toast.error('Advance Payment Commitment Date should not be empty.');
      return;
    }
    if (this.leadForm.value.ROH_RSM == null) {
      this.toast.error('ROH/RSM should not be empty.');
      return;
    }
    if (this.leadForm.value.advance_payment_remarks == null) {
      this.toast.error('Client Remarks should not be empty.');
      return;
    }
    ...
    this.dataMain = {
      ...
      ROH_RSM: val.ROH_RSM,
      fea_approval_status:val.fea_approval_status,
      status: dynamicStatus,
      status3:feaDynamicStatus,
  ...
  fun1(e: any) {
    const aa = e
    let filteredVariable = this.variable.filter((item: any) => aa.includes(item.first_name));
