...
export class ProformaInvoiceComponent {
  ...
  qnt_value: any;
  unit_price: any;
  invoice_data_list: any;
  modify_Data: any = [];
  quatation_inline_items: any;
  getPDFdata: boolean=false;
  NetAmountGrandTotal: any;
  parentSide: boolean;
  br2: any;
  singleLeadData1: any;
  contact_list: any;
  ...
  ngOnInit() {
    ...
    this.invoicePdf_List();
    this.get_cp_contact();
   
  

    this.activeroute.queryParams.subscribe((params:any) => {
      this.br_nos=params.br_Number
    //  alert(this.br_nos);
    this.getQuotationData(this.br_nos);
      this.id = params;
      this.lead_id = this.id.lead_id;
    this.InvoiceLineItems(this.lead_id,this.br_nos);
     
      this.qoutationType = params.type
      console.log(this.qoutationType);
      ...
      this.leadService.getByIdLead(this.lead_id).subscribe((res:any) => {
        ...
        } else if (this.singleLeadData.status === " ") {
        ...
        if (this.singleLeadData?.certificate_type_name === "Non-Cert") {
          ...
          console.log('non cert value--->',this.nonCertTrue);
        } else {
          this.nonCertTrue = true;
          console.log(this.nonCertTrue);
        }
        this.patchFormvalue();
        this.getCategory();
      })
    ...
    this.mainId = localStorage.getItem("EmpMainId");
      ...
      if (this.mainId != undefined) {
        this._empRegistration.getByUserId(this.mainId).subscribe((res) => {
          ...
          this.br_no=this.mainEmployeeData.br_number
          this.getAchievementList();
          this.patchFormvalue();
        })
      }
  ...
  child_br_number(arg0: string, child_br_number: any) {
    throw new Error('Method not implemented.');
  }
  updateCheck(item: any) {
    if (item.patchValue) {
      // If patch value is true, unselect the checkbox
      item.patchValue = false;
    } else {
      // If patch value is false, select the checkbox
      item.patchValue = true;
    }
  }
  ...
  invoicePdf_List(){
    this.leadService.get_invoice_list().subscribe((res:any)=>{
      this.invoice_data_list=res.data
    })
  }
...
dynamic_list(){
  ...
  add_items(e:any){
    console.log(e,'add line items');
    this.leadService.quotation_create_invoice(this.br_nos,this.modify_Data).subscribe((res:any)=>{
      console.log(res,"after create line items");
    })
    window.location.reload();
    this.toast.success("Add Item Successfully")
  }
  toPdf() {
    // window.location.reload();
  ...
  candidateClick(e:any){
    ...
    this.recruitService.interViewGetIdBy(e).subscribe((res:any)=>{
      ...
      console.log(this.interviewGetId,'interviewGetId');
      this.patchFormvalue();
    })
  }
  ...
  patchFormvalue() { 
    ...
    if(this.singleLeadData?.segment != 'null || undefined'){
      ...
      this.leadService.getCategoryBySegID(this.singleLeadData?.segment).subscribe((res:any) => {
        ...
        console.log('category list--->',this.CategorySList)
      });
    }
    ...
    if(this.singleLeadData?.region != 'null || undefined'){
      this.leadService.getRBuisnessHead(this.singleLeadData?.region).subscribe((res:any) => {
        ...
        console.log('this.regionBuisnessList', this.regionalbheadList);     
       });
       this.leadService.getGlobalManagerSales(this.singleLeadData?.region).subscribe((res:any) => {
         this.globalMsalesList = res.data;
         console.log('this.globalMsalesList', this.globalMsalesList);     
        });
    }
    ...
    if (this.singleLeadData?.city != 'undefined || null') {
      this.leadService.getPinCodeNew(this.singleLeadData?.city).subscribe((res:any) => {
        ...
        console.log('pincode list********', this.pinCodeList)
        console.log(this.pinCodeList.length);       
      });
    }
    if (this.singleLeadData?.certificate_type != 'undefined || null') {
     ...
     console.log('res for assessment id', res);
     this.assesmentList = res.data.new_spas;
     console.log('assesment list******',  this.assesmentList)
    });
    }
  ...
  submitForm() {
    ...
    this.leadService.editLead(this.lead_id, this.dataMain).subscribe((res:any) => {
      ...
      this.route.navigate(['master/lead/lead-opportunity/quotation'],
  ...
  approveBtn() {
    ...
    this.leadService.editLead(this.lead_id, data).subscribe((res:any) => {
      console.log(res);
      this.toast.success(res.message);
      this.route.navigate(['master/lead/lead-opportunity/perform-invoice'],
        { queryParams: { lead_id: this.lead_id } }
      );
    })
  }
  ...
  bookBtn() {
    ...
    this.leadService.editLead(this.lead_id, this.dataMain).subscribe((res:any) => {
      console.log(res);
      this.toast.success("Quotation Prepared Successfully..")
      this.route.navigate(['master/lead/lead-opportunity/work-order'],
        { queryParams: { lead_id: this.lead_id } }
      );
    })
  }
  ...
  InvoiceLineItems(id: any,brs:any) {
    const data={
      br_number: Number(brs)
    }
    console.log("lead id",id + "data ",data,"totototot");

    this.leadService.QuotationLineItemsChecks(id,data).subscribe((res: any) => {
      this.invoice_list = res.data
      console.log(this.invoice_list, "invoice list","totototot");
    })
  }
  add(){
  ...
  createQuotationInline() {
    console.log(this.add_dynamic_invoice);
    const data = {
      ...
      lead_generation: Number(this.lead_id),
      lineItem:false
    }
    this.leadService.quotation_create(data).subscribe((res: any) => {
      console.log(res);
      window.location.reload();
      this.toast.success("Successfully", "Create Invoice Line Items")
    })
  ...
  getQuotationData(br:any){
    this.leadService.getPdf_Inline(br).subscribe((res:any)=>{
      this.quatation_inline_items=res?.data
      console.log(this.quatation_inline_items,"quoation items");   
    })
  }
