...
export class SendQuotationComponent {
  ...
  p_request: any;
  allLogoDetails: any;
  dataset: { data: string; checked: boolean; }[];
  dataset1: { data: string; checked: boolean; }[];
  date_of_issue: any;
  ...
  respective_data = [
    {
      'value': 'DQS Holding GmbH (IATF 16949)'
    },
    {
      'value': 'DQS GmbH(DAkkS)'
    },
    {
      'value': 'DQS Inc. (ANAB)'
    },
    {
      'value': 'DQS CFS GmbH (e.g. DAkkS)'
    },
    {
      'value': 'DQS India (NABCB)'
    },
    {
      'value': 'ISO/IEC 27001'
    },
  ...
  constructor(
    ...
    this.image = '../../../../../../assets/icons/signatute.png'
  }
  ...
  ngOnInit() {
    ...
    this.activeroute.queryParams.subscribe((params: any) => {
      ...
      this.leadService.getByIdLead(this.lead_id).subscribe((res: any) => {
        ...
        console.log('this.singleLeadData 132', this.singleLeadData);
        this.singlechildLeadData = res.lenght_of_child;
        console.log('this.sing132', this.singlechildLeadData);
        this.patchFormvalue();
        this.leadForm.controls['customer_type'].disable();
        this.leadForm.controls['site_audit'].disable();

        this.date_of_issue = moment(this.singleLeadData?.lead_created_date).format('DD-MM-YYYY'),
          this.dataset = [
            {
              "data": "ISO 9001",
              "checked": false
        ...
        const requestData = {
          "dataToMatch": this.singleLeadData?.product_request
        };


        requestData.dataToMatch.forEach((value: any) => {
          const matchingItem = this.dataset.find(item => item.data === value);
          ...
          if (matchingItem) {
            ...
            const othersItem = this.dataset.find(item => item.data === "Others");
            if (othersItem) {
              othersItem.checked = true;
            }
          }

        });
        this.getStage1(this.singleLeadData?.br_number)
        this.leadService.getByIdChild(this.lead_id).subscribe((res: any) => {
          console.log('res data for child site--->', res)
          this.childsiteData = res;
          let modify: any = []
          for (let key in this.childsiteData.groupedArrays) {
            console.log(this.childsiteData.groupedArrays[key])
            modify.push(this.childsiteData.groupedArrays[key])
            console.log('modify array*****', modify)
            this.childShow = modify
          }
          console.log('this.childShow', this.childShow)
          this.location1 = this.childShow[0];
          this.location2 = this.childShow[1];
          this.location3 = this.childShow[2];
          this.location4 = this.childShow[3];
          // this.location5 = this.childShow[4];         
        })
        ...
        if (this.singleLeadData?.company_logo_req == 'No') {
          this.logoCost = false;
        }
      ...
      console.log(this.singleLeadData1,"singleLeadDataaaa");
          ...
          console.log(this.singleLeadData1, "singleLeadData");
  ...
  patchFormvalue() {
    console.log(this.singleLeadData, 'this.singleLeadData 3');
    ...
    // });
    // }
    console.log(this.singleLeadData, 'this.singleLeadData 2');
  ...
  submitForm() {
    ...
    this.dataMain = {
      ...
      // lead_created_by_name:val.lead_created_by,
  ...
  getByEmpDoc() {
    this.configService.getByEmpSign(this.mainId).subscribe((res: any) => {
      console.log(res, 'ressss documnet');
      this.signData = res.data;

      this.imgpath = res.data?.emp_signature;
    ...
    // //   signature_id: 9,
    // //   employee_id: "15",
    // //   emp_signature: "https://wallpapers.com/images/featured/hd-a5u9zq0a0ymy2dug.jpg",
    // //   status: "ACTIVE",
    // //   createdAt: "2023-10-26T10:41:37.000Z",
    // //   updatedAt: "2023-10-26T10:41:37.000Z",
    // //   employee_name: "Rahul"
    // // }
  ...
  getProfilrPhoto(url: any) {
    ...
    return url = 'https://www.freepnglogos.com/uploads/signature-png/nguy-ecnh-nguyen-van-binh-signature-png-5.png'
  }
  ...
  fetchStageDataForLeads() {
    ...
    console.log('this.stageData',this.stageData1);
