...
@Component({
  selector: 'app-fea-aaprovel-list',
  templateUrl: './fea-aaprovel-list.component.html',
  styleUrls: ['./fea-aaprovel-list.component.scss']
})
export class FeaAaprovelListComponent {
  ...
  // this.leadService.getallFeaApprovelList2().subscribe((res:any) => {

  //   this.rowData = res.result;
  //   let output = this.rowData.map((item:any) => ({ ...item, status: 'Sent FEA Approval' }));
  //   this.rowData=output
  //       console.log(output);
  ...
  this.leadService.getAllFeaApprovelNew(this.mainId).subscribe((res:any) => {

    this.rowData = res.result;
    let output = this.rowData.map((item:any) => ({ ...item, status: 'Sent FEA Approval' }));
    this.rowData=output.reverse() 
        console.log(output);
  ...
  navigate(){
    this.route.navigate(["master/lead/quotation/fea-approvel/fea-pending-list"]);
  }
  currentRout(){
