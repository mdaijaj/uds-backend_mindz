...
@Component({
  selector: 'app-fea-pending-list',
  templateUrl: './fea-pending-list.component.html',
  styleUrls: ['./fea-pending-list.component.scss']
})
export class FeaPendingListComponent {
  ...
  this.leadService.getAllFeaPendingNew(this.mainId).subscribe((res:any) => {

    this.rowData = res.result.reverse();

    for(let a=0;a<this.rowData.length;a++){
      this.rowData[a].status3="FEA Pending"
      this.rowData[a].status="Sent FEA Approval Pending"
  ...
  // this.leadService.getallFeaPendingList().subscribe((res:any) => {
