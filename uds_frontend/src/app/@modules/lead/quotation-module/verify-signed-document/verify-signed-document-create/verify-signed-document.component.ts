...
@Component({
  selector: 'app-verify-signed-document',
  templateUrl: './verify-signed-document.component.html',
  styleUrls: ['./verify-signed-document.component.scss']
})
export class VerifiedSignedDocumentNewComponent {
  ...
  dataMain: { expense_category: any; non_cert_type: any, customer_type: any; assigned_to: any; categories: any; site_audit: any; associated_company: any; segment: any; certificate_type: any; first_name: any; last_name: any; email: any; emplyoment_type: any; job_title: any; street_address: any; address2: any; city: any; state: any; country: any; postal_code: any; region: any; regional_bussiness_lead: any; global_managing_director: any; global_manager_sales: any; website_url: any; phone_number: any; mobile_number: any; dqs_contact_source: any; contact_owner: any; lead_created_date: string; standard_program_assement: any; remarks: any;
    ...
    opp_verified_remarks_level1: any; reject_remarks_a: any; reject_remarks_b: any; opp_verifier_name_level2: any; opp_verified_date_level2: string; opp_verified_remarks_level2: any; quotation_currency: number; slab_quote: any;totalAmount_inInr:any; logo_cost: any; quote_prepared: any; quote_prepared_date: string; status: string; company_logo_req:any; company_logo_cost:any;
    agreed_slab_a: any;
    agreed_slab_b: any;
  ...
  constructor(
    ...
    this.leadForm = this.fb.group({
      ...
      verfivation_remarks: new FormControl(null),
...
this.logoCost=true;
  ...
  rejectNew() {
    Swal.fire({
      title: 'Are you sure to Reject Verify Documents Detail ?',
      ...
      if (result.isConfirmed) {
        const data = {
          status: this.singleLeadData.certificate_type_name =='Cert'? "Digitally Signed Document" : "Documents Signed Uploaded"

        }
        this.leadService.editLead(this.lead_id, data).subscribe((res: any) => {
    
          this.toast.success("Reject Successfuly");
  ...
  submitQAForm() {
    ...
    this.leadService.editLead(this.lead_id, this.dataMain).subscribe((res:any) => {
      ...
      // this.raiseWorkOrder();
      this.toast.success("Documents Verified Successfully..")
      this.route.navigate(['master/lead/quotation'],
        { queryParams: { lead_id: this.lead_id } }
      );
    })
  ...
  cancel(){
    this.route.navigate(['master/lead/quotation/verify-signed-document'])
  }
  ...
  viewDoc(e:any) {
   ...
   downloadOthers(e: any) {
     const pdfUrl = e.upload_doc;
     const pdfName = e.file_name;
     FileSaver.saveAs(pdfUrl, pdfName);
   }
    ...
    if(e.document_status != 'Verified'){
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          this.leadService.doc_Verify(e.upload_doc_id,data).subscribe((res:any)=>{
            if(res.code ==200){
              this.toast.success('Document Verified Successfuly')
              this.getDocs();
            }
          },(err)=>{
      ...
      this.toast.error('Already Document Verify')
    }
   }

   docs_not_verify(e:any){
    if(e.document_status != 'Not Verified'){
      Swal.fire({
        title: 'Are you sure to Not verify this document',
        text: "You won't be able to revert this!",
        icon: 'warning',
        showCancelButton: true,
        cancelButtonColor: '#f44336',
      ...
      this.toast.error('Already Document Not Verify')
    }
   ...
   getDocs(){
    this.leadService.getdocById(this.lead_id).subscribe((res:any)=>{
      ...
      for(let i=0;i<=res.data.length-1;i++){
        statusDoc.push(res.data[i]?.document_status)
      }
console.log(statusDoc,'statusDoc');
this.statusDocment=statusDoc.every((element:any) => element === "Verified");
console.log(this.statusDocment,'this.statusDocment');
