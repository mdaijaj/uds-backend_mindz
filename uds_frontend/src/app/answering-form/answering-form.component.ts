...
@Component({
  selector: 'app-answering-form',
  templateUrl: './answering-form.component.html',
  styleUrls: ['./answering-form.component.scss']
})
export class AnsweringFormComponent {
  leadForm: FormGroup;
  jobType: any;
  candidateArray: any;
  interviewGetId: any;
  ...
  submitForm() {
    ...
    this.leadService.updateRequest(this.lead_id, data).subscribe((res: any) => {
      ...
      this.route.navigate(['response-confirmation'],
  ...
  getlead(){
    this.leadService.leadData_br_number(this.singleLeadData.inter_company_br_number).subscribe((res:any)=>{
      ...
      let auditData=[{name:this.lead_data?.auditor_name, no: this.singleLeadData.phone1}]
      console.log(auditData,"Vrajesh");
      
      
      this.leadForm.patchValue({
        Ref_No: this.singleLeadData?.ref_no_s,
        Standards: this.lead_data?.product_request,
        Leading_DQS_office:this.lead_data?.associated_company,
        Answering_DQS_office:this.singleLeadData?.name_legal_status,
        Req_Date:this.singleLeadData?.due_date,
        Req_Date_audit:this.singleLeadData?.due_date,
        name:this.singleLeadData?.auditor_name,
      })
      console.log('this.CF_1', this.CF_1);
      for(let i=0; i<= auditData?.length ; i++ ){
        this.CF_1.Auditor.push(
          new FormGroup({
            name: new FormControl(
            auditData[i].name
            ),
            audit_no: new FormControl(
              auditData[i].no
            ),
            audit_roll:  new FormControl('Auditor'),
          })
        );
      }
      console.log(this.CF_1,"chkkkkkkk");
    })
  }
}
