...
exports.signup = async (req, res) => {
  try {
    ...
    if (employee_official_email && first_name) {
    ...
    if (first_name && employee_official_email) {
      var chars = "0123456789abcdefghijklmnopqrstuvwxyz!@#$%^&*()ABCDEFGHIJKLMNOPQRSTUVWXYZ";
      var passwordLength = 14;
      var password = "";
      for (var i = 0; i <= passwordLength; i++) {
      ...
      if (req.body.employee_id != 'null') {
        ...
        const bytesPassword = CryptoJS.AES.decrypt(password, 'password');
        decryptPassword = bytesPassword.toString(CryptoJS.enc.Utf8);
        if (decryptPassword) {
          info = await transport.mailsend({
            from: process.env.EMAIL_FROM,
            to: employee_official_email,
            ...
            html: `<p><strong>Hi ${first_name}</strong> <br>Please find your link for ERP login <a href = ${link}>Click Here </a>
         Your <strong>Official Email Id is</strong> ${employee_official_email}
         Your Password is: <strong> ${decryptPassword} </strong> </p>`,
          });
        }
...
exports.get_ById_user = async (req, res) => {
  try {
    ...
    const getData = await User.findAll({ where: { employee_id: employeeId } });
    if (getData) {
      var array = [];
...
exports.signin = async (req, res) => {
  ...
  let isInActive = await User.findOne({
    where: {
  ...
  if (req.body.employee_official_email == employee_official_email && req.body.password == userPassword) {
    ...
    User.findOne({
      ...
      // var passwordIsValid = req.body.password === defaultPassword || bcrypt.compareSync(req.body.password, user.password);

      const bytesUserPassword = CryptoJS.AES.decrypt(user.password, 'password');
      const userDecryptPassword = bytesUserPassword.toString(CryptoJS.enc.Utf8);
      if (req?.body?.password != userDecryptPassword) {
      ...
      FROM role_module_masters rmm
      LEFT JOIN menu_masters mm ON rmm.role_module_master_id = mm.role_module_master_id
      LEFT JOIN submenu_masters sm ON mm.menu_master_id=sm.menu_master_id
      LEFT JOIN role_menu_accesses rma ON (rma.role_module_master_id = rmm.role_module_master_id OR rma.role_module_master_id IS NULL) AND 
...
exports.get_All_Aduitorlistwithname = async (req, res) => {
  try {
    const userData = await User.findAll({
      ...
      attributes: ["employee_id", "employee_code", "first_name", "employee_official_email", "mobile_number", "designation", "role_master_id"],
    });
    if (userData) {
...
exports.userById = async (req, res) => {
  try {
    ...
    if (!jsonData) {
      ...
      const jsonData2 = await User.findOne({ where: { employee_id: jsonData.dataValues.reporting_manager_id } })
      if (jsonData2) {
        let manager_name = jsonData2.first_name + " " + jsonData2.last_name
        jsonData.dataValues.reporting_manager = manager_name
      }
...
exports.userDetailDelete = async (req, res) => {
  try {
    ...
    if (userDetails) {
      ...
      return res.status(200).send({ code: 200, message: "User deleted Successfully!", data: employData });
    } else {
      return res.status(404).send({ code: 404, message: "Record Not Found" });
    }
  } catch (error) {
...
exports.employee_data = async (req, res) => {
  try {
    ...
    console.log("employee data 2 is", emp_data2)
    if (!emp_data2) {
...
exports.generate_Employee_Code = async (req, res) => {
  try {
    ...
    if (first_name === "") {
      return res.status(200).send({ code: 200, message: "Please Fill your first_Name First", data: null });
    } else {
      return res.status(200).send({ code: 200, message: "Employee Code has been Generated For Registered User", data: employeeCode });
    }
  } catch (error) {
    console.log(error);
    return res.status(500).send({ code: 500, message: "Internal Server Error" });
...
exports.get_all_employeeBy_User_role = async (req, res) => {
  try {
    ...
    const getAllData = await User.findAll({
      where: { role_master_id: role_Id, status: 'ACTIVE', isDeleted: false },
...
exports.changePassword = async (req, res) => {
  try {
    const { employee_id, password, oldPassword } = req.body;
    const findUser = await User.findOne({
      where: { employee_id: employee_id }
    });
    if (!findUser) {
      return res.status(403).send({ code: 403, message: "User Not Found" });
    }
    else {
      const bytesPassword = CryptoJS.AES.decrypt(findUser?.password, 'password');
      const userPassword = bytesPassword.toString(CryptoJS.enc.Utf8);
      if (userPassword != oldPassword) {
        return res.status(403).send({ code: 403, message: "Old password not currect" });
      }
      else {
        let updatedData = await User.update({
          password: password
        },
          { where: { employee_id: employee_id } })
        if (updatedData) {
          const bytesChangeassword = CryptoJS.AES.decrypt(password, 'password');
          const changedPassword = bytesChangeassword.toString(CryptoJS.enc.Utf8);
          let webLink = "https://emerp.elitetraveltech.in/auth/login";
          info = await transport.mailsend({
            from: "dqsindia.erp@gmail.com",
            to: findUser?.employee_official_email,
            subject: "ERP - Password ",
            html: `<p><strong>Hi ${findUser?.first_name}</strong> <br>Please find your link for ERP login <a href = ${webLink}>Click Here </a>
           Your <strong>Official Email Id is</strong> ${findUser?.employee_official_email}
           Your Changed Password is: <strong> ${changedPassword} </strong> </p>`,
          });

          return res.status(200).send({ code: 200, message: "Password successfully changed" });
        }
      }
    }
  } catch (error) {
...
exports.forgotSendOtp = async (req, res) => {
  try {
    const { employee_official_email, baseUiUrl } = req.body;
    const findUser = await User.findOne({
      where: { employee_official_email: employee_official_email }
    });
    if (!findUser) {
      return res.status(403).send({ code: 403, message: "Invalid Email Id" });
    }
    else {
      let webLink = `${baseUiUrl}/auth/login/forgot-password`;
      const encryptedUserId = CryptoJS.AES.encrypt(findUser.employee_id.toString(), 'userId').toString();
      info = await transport.mailsend({
        from: "dqsindia.erp@gmail.com",
        to: findUser?.employee_official_email,
        subject: "ERP - Password ",
        html: `<p><strong>Hi ${findUser?.first_name}</strong> <br>Please use this link for forgot password <a href = ${webLink}?id=${encryptedUserId}>Click Here </a>
       Your <strong>Official Email Id is</strong> ${findUser?.employee_official_email} </p>`,
      });

      return res.status(200).send({ code: 200, message: "Link send on your mail please check" });
    }
  } catch (error) {
    console.log(error);
    return res.status(500).send({ code: 500, message: "Server Error" });
...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    const bytesUserId = CryptoJS.AES.decrypt(userId, 'userId');
    const decryptUserId = bytesUserId.toString(CryptoJS.enc.Utf8);
    const findUser = await User.findOne({
      where: { employee_id: decryptUserId }
    });
    if (!findUser) {
      return res.status(403).send({ code: 403, message: "User Not Found" });
    }
